 ...
 * @author shanki
 */
public class PasSMetaSolver extends BaseGroundMetaSolver {
    
    public PasSMetaSolver(Solver solver) {
        super(PasSMetaInterpreter, solver);
    }
    
    private static final Program PasSMetaInterpreter;
    private static final String raw = "" +
        ...
        "has_negative_body(R) :- nbl(X,R)." +
        "closure_rule(R) :- rule(R), not has_negative_body(R)." +
    ...
    static {
        try {
            PasSMetaInterpreter = new ProgramFactory().fromString(raw);
