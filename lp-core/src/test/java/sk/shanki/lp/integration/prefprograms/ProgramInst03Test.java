/*
 * Copyright shanki. All rights reserved.
 */
package sk.shanki.lp.integration.prefprograms;

import java.io.IOException;
import org.junit.AfterClass;
import static org.junit.Assert.*;
import org.junit.BeforeClass;
import sk.shanki.lp.AnswerSet;
import sk.shanki.lp.AnswerSets;
import sk.shanki.lp.Atom;
import sk.shanki.lp.FunctionalTerm;
import sk.shanki.lp.Neg;
import sk.shanki.lp.Program;
import sk.shanki.lp.solvers.DisjunctiveTransSolver;
import sk.shanki.lp.gsolver.PasGTransSolver;
import sk.shanki.lp.meta.PasCnfMetaSolver;
import sk.shanki.lp.meta.PasDMetaSolver;
import sk.shanki.lp.meta.PasDSTMetaSolver;
import sk.shanki.lp.meta.PasNoMetaSolver;
import sk.shanki.lp.meta.PasSMetaSolver;
import sk.shanki.lp.meta.PasWZLMetaSolver;
import sk.shanki.lp.parser.ProgramFactory;
import sk.shanki.lp.solvers.BuiltInSmodelsSolver;
import sk.shanki.lp.solvers.NonGroundSolver;
import sk.shanki.lp.solvers.Solver;
import sk.shanki.lp.exceptions.SolverException;

/**
 *
 * @author shanki
 */
public class ProgramInst03Test {
    
    private static Program program;
    private static AnswerSet as1;
    private static AnswerSet as2;
    private static AnswerSet as3;
    private static Solver solver;
        
    @BeforeClass
    public static void setUpClass() throws SolverException, IOException {
        program = new ProgramFactory().fromString(
                "safe(a)." + 
                "nice(b)." +
                "fast(c)." +
                "@name(r4) sel(X) :- safe(X), not -sel(X)." +
                "@name(r5) sel(X) :- nice(X), not -sel(X)." +
                "@name(r6) sel(X) :- fast(X), not -sel(X)." +
                "-sel(b) :- not sel_many, sel(a)." +
                "-sel(c) :- not sel_many, sel(a)." +
                "-sel(a) :- not sel_many, sel(b)." +
                "-sel(c) :- not sel_many, sel(b)." +
                "-sel(a) :- not sel_many, sel(c)." +
                "-sel(b) :- not sel_many, sel(c)." +                        
                "r5 < r4." +
                "r6 < r5."
        );
        
        as1   = new AnswerSet();
        as1.add(new Atom("safe", new FunctionalTerm("a")));
        as1.add(new Atom("nice", new FunctionalTerm("b")));
        as1.add(new Atom("fast", new FunctionalTerm("c")));
        as1.add(new Atom("sel", new FunctionalTerm("a")));
        as1.add(new Neg(new Atom("sel", new FunctionalTerm("b"))));
        as1.add(new Neg(new Atom("sel", new FunctionalTerm("c"))));

        as2   = new AnswerSet();
        as2.add(new Atom("safe", new FunctionalTerm("a")));
        as2.add(new Atom("nice", new FunctionalTerm("b")));
        as2.add(new Atom("fast", new FunctionalTerm("c")));
        as2.add(new Neg(new Atom("sel", new FunctionalTerm("a"))));
        as2.add(new Atom("sel", new FunctionalTerm("b")));
        as2.add(new Neg(new Atom("sel", new FunctionalTerm("c"))));

        as3   = new AnswerSet();
        as3.add(new Atom("safe", new FunctionalTerm("a")));
        as3.add(new Atom("nice", new FunctionalTerm("b")));
        as3.add(new Atom("fast", new FunctionalTerm("c")));
        as3.add(new Neg(new Atom("sel", new FunctionalTerm("a"))));
        as3.add(new Neg(new Atom("sel", new FunctionalTerm("b"))));
        as3.add(new Atom("sel", new FunctionalTerm("c")));
        
        solver = new NonGroundSolver(new BuiltInSmodelsSolver());
    }
    
    @AfterClass
    public static void tearDownClass() {
        program = null;
        as1     = null;
        as2     = null;
        as3     = null;
    }

    public void testAS() throws SolverException {        
        AnswerSets expected = new AnswerSets();
        expected.add(as1);
        expected.add(as2);
        expected.add(as3);
        
        AnswerSets result   = solver.evaluate(program, 0);
        
        assertEquals(expected, result);
    }

//  Took too long
//    @Test
//    public void testPAS_BE() throws RuleSolverException {
//        AnswerSet as1   = new AnswerSet();
//        as1.add(new Atom("safe", "a"));
//        as1.add(new Atom("nice", "b"));
//        as1.add(new Atom("fast", "c"));
//        as1.add(new Atom("sel", "a"));
//        as1.add(new Atom(true, "sel", "b"));
//        as1.add(new Atom(true, "sel", "c"));
//
//        AnswerSet as2   = new AnswerSet();
//        as2.add(new Atom("safe", "a"));
//        as2.add(new Atom("nice", "b"));
//        as2.add(new Atom("fast", "c"));
//        as2.add(new Atom(true, "sel", "a"));
//        as2.add(new Atom("sel", "b"));
//        as2.add(new Atom(true, "sel", "c"));
//
//        AnswerSet as3   = new AnswerSet();
//        as3.add(new Atom("safe", "a"));
//        as3.add(new Atom("nice", "b"));
//        as3.add(new Atom("fast", "c"));
//        as3.add(new Atom(true, "sel", "a"));
//        as3.add(new Atom(true, "sel", "b"));
//        as3.add(new Atom("sel", "c"));
//        
//        AnswerSets expected = new AnswerSets();
//        expected.add(as1);
//        expected.add(as2);
//        expected.add(as3);
//        
//        
//        AnswerSets result   = program.metaEvaluate(new PasBEMetaInterpreter());
//        
//        assertEquals(expected, result);
//    }    

    public void testPAS_DST() throws SolverException {
        AnswerSets expected = new AnswerSets();
        expected.add(as1);
        
        AnswerSets result   = new NonGroundSolver(new PasDSTMetaSolver(solver)).evaluate(program, 0);
        
        assertEquals(expected, result);
    }

    public void testPAS_WZL() throws SolverException {
        AnswerSets expected = new AnswerSets();
        expected.add(as1);
        
        AnswerSets result   = new NonGroundSolver(new PasWZLMetaSolver(solver)).evaluate(program, 0);
        
        assertEquals(expected, result);
    }    

    public void testPAS_DIRECT() throws SolverException {
        AnswerSets expected = new AnswerSets();
        expected.add(as1);
        expected.add(as2);
        expected.add(as3);
        
        AnswerSets result   = new NonGroundSolver(new PasDMetaSolver(solver)).evaluate(program, 0);
        
        assertEquals(expected, result);
    }

    public void testPAS_INDIRECT_TRANS() throws SolverException {
        AnswerSets expected = new AnswerSets();
        expected.add(as1);
        
        AnswerSets result   = new NonGroundSolver(new PasGTransSolver(solver,new DisjunctiveTransSolver(solver))).evaluate(program, 0);
        
        assertEquals(expected, result);
    }

    public void testPAS_NO_CONFLICT() throws SolverException {
        AnswerSets expected = new AnswerSets();
        expected.add(as1);
        
        AnswerSets result   = new NonGroundSolver(new PasNoMetaSolver(solver)).evaluate(program, 0);
        
        assertEquals(expected, result);
    }

    public void testPAS_STRICT() throws SolverException {
        AnswerSets expected = new AnswerSets();
        expected.add(as1);
        expected.add(as2);
        expected.add(as3);
        
        AnswerSets result   = new NonGroundSolver(new PasSMetaSolver(solver)).evaluate(program, 0);
        
        assertEquals(expected, result);
    }    

    public void testPAS_CNF() throws SolverException {
        AnswerSets expected = new AnswerSets();
        expected.add(as1);
        
        AnswerSets result   = new NonGroundSolver(new PasCnfMetaSolver(solver)).evaluate(program, 0);
        
        assertEquals(expected, result);
    }

}